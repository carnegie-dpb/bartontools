...
plot.compare.fits = function(df) {

    ymin = 1e-8

    earlies = df$logFC.group=="E"
    middles = df$logFC.group=="M"
    lates   = df$logFC.group=="L"
    
    plot(df$R2.E[earlies & df$R2.E>0], df$q.30[earlies& df$R2.E>0], log="y", xlim=c(0,1), ylim=c(ymin,1), pch=19, cex=0.2, col="red",
         xlab=bquote(paste("model fit coefficient of determination ",r^2)), ylab="q-value (B-H method)"
         )
    points(df$R2.M[middles & df$R2.M>0], df$q.60[middles & df$R2.M>0], pch=19, cex=0.2, col="darkgreen")
    points(df$R2.L[lates & df$R2.L>0], df$q.120[lates & df$R2.L>0], pch=19, cex=0.2, col="blue")
    ...
    lines(c(0,1), c(0.05,0.05),  col="black", lty="dashed", lwd=2)
    lines(c(0.6,0.6), c(ymin,1), col="black", lty="dashed", lwd=2)

    ## count those with r^2>0.6 that have q above/below 0.05
    ...
    rect(c(0.65,0.77,0.89), 0.06, c(0.75,0.86,0.98), 0.135, col="white", border=NA)
    rect(c(0.65,0.77,0.89), 0.017, c(0.75,0.86,0.98), 0.04, col="white", border=NA)

    text(x=c(0.70,0.82,0.94), y=0.09, col=c("red","darkgreen","blue"), cex=1.5, font=1,
         labels=c(
