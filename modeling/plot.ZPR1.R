...
plot.ZPR1 = function() {
    ...
    rhop = rhop(t=t, rhoc0=rhoc0, rhon0=rhon0, nu=nu, rhop0=rhop0, etap=etap, gammap=gammap)

    ## plot starts
    
    plot(t, log2(rhop/rhop0), type="l", xlab="time after DEX application (h)", ylab=expression(paste(log[2],"(relative values)")), ylim=c(-0.5,5))
    points(c(0,0.5,1,2), logFC, pch=19, cex=2, col="red")

    lines(c(0,max(t)), c(logFCinf,logFCinf), lty=2)
    points(times, log2(ZPR1/rhop0), pch=3, cex=2, col="blue")
    ...
    legend(max(t), y1-0.25*ydelta, xjust=1, yjust=1, pch=c(3,19,NA), col=c("blue","red","black"), pt.cex=2, lty=c(NA,NA,1), cex=1.2,
           c(
