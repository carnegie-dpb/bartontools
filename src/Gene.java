...
public class Gene implements Comparable {
	...
	public static String getIdForName(DB db, String name) throws SQLException {
		db.executeQuery("SELECT * FROM public.genes WHERE name="+Util.charsOrNull(name)+" ORDER BY id");
		if (db.rs.next()) {
	...
	public static String[] getIDsForName(DB db, String name, Experiment experiment) throws SQLException {
		TreeSet<String> set = new TreeSet<String>();
		db.executeQuery("SELECT * FROM public.genes WHERE genus='"+experiment.genus+"' AND species='"+experiment.species+"' AND name="+Util.charsOrNull(name));
	...
	public static String[] getIDsForAlias(DB db, String name, Experiment experiment) throws SQLException {
		TreeSet<String> set = new TreeSet<String>();
		db.executeQuery("SELECT * FROM public.genealiases WHERE genus='"+experiment.genus+"' AND species='"+experiment.species+"' AND name="+Util.charsOrNull(name));
		while (db.rs.next()) set.add(db.rs.getString("id"));
		return set.toArray(new String[0]);
	}
	/**
	...
	/**
	 * Return public.genes which have descriptions containing the supplied string; unique, sorted.
	 */
	public static Gene[] searchOnDescriptions(ServletContext context, Experiment experiment, String term) throws SQLException, FileNotFoundException, NamingException, ClassNotFoundException, UnsupportedEncodingException {
		...
		try {
			db = new DB(context);
			TreeSet<Gene> set = new TreeSet<Gene>();
			db.executeQuery("SELECT * FROM public.genes WHERE genus='"+experiment.genus+"' AND species='"+experiment.species+"' AND description ILIKE '%"+term+"%' LIMIT 1000");
